...
let msg = {
  ...
  if (msg) return conn.sendMessage(m.chat, {
    ...
    let daftar = {
  unreg: `Untuk dapat mengakses seluruh fitur Bot,\nAnda harus melakukan \`\`\`DAFTAR\`\`\` terlebih dahulu.\nIntuk melakukan _daftar_ caranya cukup mudah\nketik: /daftar nama.umur\ncara lainnya:\n*ketik: @verify atau cara mudah tinggal klik dibawah ini*`}[type]
  
  let beton = [
{
    "name": "quick_reply",
    "buttonParamsJson": `{"display_text": "DAFTAR klik ini kak", "id": "@verify"}`
    }
]

  if (daftar) return conn.sendUrlImageButton(m.chat, daftar, beton, wm, registrasi)
}
